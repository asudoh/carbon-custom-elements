...
export default (config: DatePickerMonthSelectPluginConfig): Plugin => (fp: FlatpickrInstance) => {
  ...
  const updateCurrentMonth = () => {
    const { monthElements, currentMonth, l10n } = fp;
    const monthStr = monthToStr(currentMonth, config.shorthand === true, l10n);
  ...
  return {
    onMonthChange: updateCurrentMonth, // Fires on prev/next month navigation
    onValueUpdate: updateCurrentMonth, // Fires on manual enter of date
    onReady: [register, setupElements, updateCurrentMonth],
  };
};
